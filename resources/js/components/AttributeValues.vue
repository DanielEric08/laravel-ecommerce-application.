...
<script>
    export default {
        ...
        methods: {
            loadValues() {
                let attribute_Id = this.attribute_id;
                let _this = this;
            ...
            saveValue() {
                if (this.value === '') {
                    this.$sweetalert("Error, Value for attribute is required.", {
                       icon: "error",
                    });
                } else {
                    let attribute_Id = this.attribute_id;
                    ...
                    axios.post('/admin/attributes/add-values', {
                        ...
                        _this.resetValue();
                        _this.$sweetalert("Success! Value added successfully!", {
                            icon: "success",
                        });
                    }).catch(function (error) {
                        console.log(error);
            ...
            updateValue() {
                if (this.value === '') {
                    ...
                    axios.post('/admin/attributes/update-values', {
                        ...
                        _this.resetValue();
                        _this.values.push(response.data);
                        _this.$sweetalert("Success! Value updated successfully!", {
                            icon: "success",
                        });
                    }).catch(function (error) {
                        console.log(error);
                    ...
                    if (willDelete) {
                        ...
                        axios.post('/admin/attributes/delete-values', {
                            ...
                            if (response.data.status === 'success') {
                                _this.values.splice(_this.key, 1);
                                _this.resetValue();
                                _this.$sweetalert("Success! Attribute value has been deleted!", {
                                    icon: "success",
                                });
                            } else {
                                _this.$sweetalert("Your attribute value not deleted!");
                            }
                        }).catch(function (error) {
                            console.log(error);
                        });
                    } else {
                        this.$sweetalert("Your option value not deleted!");
                    }
                });
            },
            resetValue() {
